...
for(let n=1;n<=3;n++){
 ...
 assert.equal(await page.locator("#speech-record").isDisabled(),true);
 assert.match(await page.locator("#speech-status").innerText(),/being set up/);
...
console.log("Browser checks passed: 3 units, mobile overflow, setup state, 79.99 failure, incomplete reading failure, 80 pass, next-word lock, reload progress, recording cancel. Synthetic microphone and mocked Azure response only.");
