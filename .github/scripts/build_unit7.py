...
unit_dir = root / "unit-07"
data = json.loads((unit_dir / "unit-07-content.json").read_text(encoding="utf-8"))
source_root = root.parents[1]
shutil.copy2(source_root / "Track 13.mp3", unit_dir / "Track 13.mp3")
shutil.copy2(source_root / "Track 14.mp3", unit_dir / "Track 14.mp3")

template = (root / "unit-04/unit-04-quiz.html").read_text(encoding="utf-8")
html = re.sub(r"<title>[\s\S]*?</title>", "<title>Banning Free Plastic Bags Vocabulary &amp; Reading Quiz</title>", template, count=1)
html = re.sub(r"<h1>[\s\S]*?</h1>", "<h1>Banning Free Plastic Bags</h1>", html, count=1)
html = html.replace("Reading — Track 07", "Reading — Track 13").replace('src="Track 07.mp3"', 'src="Track 13.mp3"')
html = html.replace("Vocabulary — Track 08", "Vocabulary — Track 14").replace('src="Track 08.mp3"', 'src="Track 14.mp3"')
...
(unit_dir / "unit-07-quiz.html").write_text(html, encoding="utf-8")
...
title = doc.add_heading("LiveABC Basic 2000 Words — Unit 7", 0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("Listen to Track 13 and fill in the 18 missing words.")
...
doc.add_page_break()
doc.add_heading("Teacher Answer Key — Unit 7", 0)
...
for i, item in enumerate(data["vocabulary"], 1):
    ...
    run.font.size = Pt(8)
doc.save(unit_dir / "unit-07-test.docx")

sent_path = root / "scripts/speech-sentences.json"
sent = json.loads(sent_path.read_text(encoding="utf-8"))
sent["7"] = [item["sample_sentence"] for item in data["vocabulary"]]
sent_path.write_text(json.dumps(sent, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

index_path = root / "index.html"
index = index_path.read_text(encoding="utf-8")
if "UNIT 07" not in index:
    card = '    <section class="card"><div class="tag">UNIT 07</div><h2>Banning Free Plastic Bags</h2><p>Listening, reading, vocabulary, and target-word spelling and optional read-aloud practice.</p><a class="button" href="unit-07/unit-07-quiz.html">Start Unit 7</a><a class="download" href="unit-07/unit-07-quiz.html#speech">Spell & read aloud · 拼字與朗讀</a><a class="download" href="unit-07/unit-07-test.docx">Download Word test</a></section>\n'
    index = index.replace('  </div>\n  <p class="note">', card + '  </div>\n  <p class="note">')
    index_path.write_text(index, encoding="utf-8")

readme_path = root / "README.md"
readme = readme_path.read_text(encoding="utf-8")
if "Unit 7 —" not in readme:
    readme = readme.replace("- [Unit 6 — Starting Seventh Grade](unit-06/unit-06-quiz.html)", "- [Unit 6 — Starting Seventh Grade](unit-06/unit-06-quiz.html)\n- [Unit 7 — Banning Free Plastic Bags](unit-07/unit-07-quiz.html)")
readme = readme.replace("Units 1–6", "Units 1–7")
readme_path.write_text(readme, encoding="utf-8")

for rel in ("scripts/validate-speaking.cjs", "scripts/browser-test.cjs"):
    path = root / rel
    text = path.read_text(encoding="utf-8").replace("n<=6", "n<=7")
    if rel.endswith("browser-test.cjs"):
        text = text.replace("count(),6", "count(),7")
    if rel.endswith("validate-speaking.cjs"):
        text = text.replace("Validated 6 units", "Validated 7 units")
