...
unit_dir = root / "unit-20"
data = json.loads((unit_dir / "unit-20-content.json").read_text(encoding="utf-8"))
source_root = root.parents[1]
shutil.copy2(source_root / "Track 39.mp3", unit_dir / "Track 39.mp3")
shutil.copy2(source_root / "Track 40.mp3", unit_dir / "Track 40.mp3")

template = (root / "unit-04/unit-04-quiz.html").read_text(encoding="utf-8")
html = re.sub(r"<title>[\s\S]*?</title>", "<title>Rosefield Tour Bus Vocabulary &amp; Reading Quiz</title>", template, count=1)
html = re.sub(r"<h1>[\s\S]*?</h1>", "<h1>Rosefield Tour Bus</h1>", html, count=1)
html = html.replace("Reading — Track 07", "Reading — Track 39").replace('src="Track 07.mp3"', 'src="Track 39.mp3"')
html = html.replace("Vocabulary — Track 08", "Vocabulary — Track 40").replace('src="Track 08.mp3"', 'src="Track 40.mp3"')
...
(unit_dir / "unit-20-quiz.html").write_text(html, encoding="utf-8")
...
title = doc.add_heading("LiveABC Basic 2000 Words — Unit 20", 0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("Listen to Track 39 and fill in the 13 missing words.")
...
doc.add_page_break()
doc.add_heading("Teacher Answer Key — Unit 20", 0)
...
for i, item in enumerate(data["vocabulary"], 1):
    ...
    run.font.size = Pt(8)
doc.save(unit_dir / "unit-20-test.docx")

sent_path = root / "scripts/speech-sentences.json"
sent = json.loads(sent_path.read_text(encoding="utf-8"))
sent["20"] = [item["sample_sentence"] for item in data["vocabulary"]]
sent_path.write_text(json.dumps(sent, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

index_path = root / "index.html"
index = index_path.read_text(encoding="utf-8")
if "UNIT 20" not in index:
    card = '    <section class="card"><div class="tag">UNIT 20</div><h2>Rosefield Tour Bus</h2><p>Listening, reading, vocabulary, and target-word spelling and optional read-aloud practice.</p><a class="button" href="unit-20/unit-20-quiz.html">Start Unit 20</a><a class="download" href="unit-20/unit-20-quiz.html#speech">Spell & read aloud · 拼字與朗讀</a><a class="download" href="unit-20/unit-20-test.docx">Download Word test</a></section>\n'
    index = index.replace('  </div>\n  <p class="note">', card + '  </div>\n  <p class="note">')
    index_path.write_text(index, encoding="utf-8")

readme_path = root / "README.md"
readme = readme_path.read_text(encoding="utf-8")
if "Unit 20 —" not in readme:
    readme = readme.replace("- [Unit 19 — Bear River High School Flea Market](unit-19/unit-19-quiz.html)", "- [Unit 19 — Bear River High School Flea Market](unit-19/unit-19-quiz.html)\n- [Unit 20 — Rosefield Tour Bus](unit-20/unit-20-quiz.html)")
readme = readme.replace("Units 1–19", "Units 1–20")
readme_path.write_text(readme, encoding="utf-8")

for rel in ("scripts/validate-speaking.cjs", "scripts/browser-test.cjs"):
    path = root / rel
    text = path.read_text(encoding="utf-8").replace("n<=19", "n<=20")
    if rel.endswith("browser-test.cjs"):
        text = text.replace("count(),19", "count(),20")
    if rel.endswith("validate-speaking.cjs"):
        text = text.replace("Validated 19 units", "Validated 20 units")
