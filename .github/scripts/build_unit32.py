...
unit_dir = root / "unit-32"
data = json.loads((unit_dir / "unit-32-content.json").read_text(encoding="utf-8"))
source_root = root.parents[1]
shutil.copy2(source_root / "Track 63.mp3", unit_dir / "Track 63.mp3")
shutil.copy2(source_root / "Track 64.mp3", unit_dir / "Track 64.mp3")

template = (root / "unit-04/unit-04-quiz.html").read_text(encoding="utf-8")
html = re.sub(r"<title>[\s\S]*?</title>", "<title>Bruce and the Spider Vocabulary &amp; Reading Quiz</title>", template, count=1)
html = re.sub(r"<h1>[\s\S]*?</h1>", "<h1>Bruce and the Spider</h1>", html, count=1)
html = html.replace("Reading — Track 07", "Reading — Track 63").replace('src="Track 07.mp3"', 'src="Track 63.mp3"')
html = html.replace("Vocabulary — Track 08", "Vocabulary — Track 64").replace('src="Track 08.mp3"', 'src="Track 64.mp3"')
...
(unit_dir / "unit-32-quiz.html").write_text(html, encoding="utf-8")
...
title = doc.add_heading("LiveABC Basic 2000 Words — Unit 32", 0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("Listen to Track 63 and fill in the 26 missing words.")
...
doc.add_page_break()
doc.add_heading("Teacher Answer Key — Unit 32", 0)
...
for i, item in enumerate(data["vocabulary"], 1):
    cell = table.cell((i - 1) % 13, (i - 1) // 13)
    run = cell.paragraphs[0].add_run(f'{i}. {item["word"]}: {item["sample_sentence"]}')
    run.font.size = Pt(8)
doc.save(unit_dir / "unit-32-test.docx")

sent_path = root / "scripts/speech-sentences.json"
sent = json.loads(sent_path.read_text(encoding="utf-8"))
sent["32"] = [item["sample_sentence"] for item in data["vocabulary"]]
sent_path.write_text(json.dumps(sent, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

index_path = root / "index.html"
index = index_path.read_text(encoding="utf-8")
if "UNIT 32" not in index:
    card = '    <section class="card"><div class="tag">UNIT 32</div><h2>Bruce and the Spider</h2><p>Listening, reading, vocabulary, and target-word spelling and optional read-aloud practice.</p><a class="button" href="unit-32/unit-32-quiz.html">Start Unit 32</a><a class="download" href="unit-32/unit-32-quiz.html#speech">Spell & read aloud · 拼字與朗讀</a><a class="download" href="unit-32/unit-32-test.docx">Download Word test</a></section>\n'
    index = index.replace('  </div>\n  <p class="note">', card + '  </div>\n  <p class="note">')
    index_path.write_text(index, encoding="utf-8")

readme_path = root / "README.md"
readme = readme_path.read_text(encoding="utf-8")
if "Unit 32 —" not in readme:
    readme = readme.replace("- [Unit 31 — Joining a Club](unit-31/unit-31-quiz.html)", "- [Unit 31 — Joining a Club](unit-31/unit-31-quiz.html)\n- [Unit 32 — Bruce and the Spider](unit-32/unit-32-quiz.html)")
readme = readme.replace("Units 1–31", "Units 1–32")
readme_path.write_text(readme, encoding="utf-8")

for rel in ("scripts/validate-speaking.cjs", "scripts/browser-test.cjs"):
    path = root / rel
    text = path.read_text(encoding="utf-8").replace("n<=31", "n<=32")
    if rel.endswith("browser-test.cjs"):
        text = text.replace("count(),31", "count(),32")
    if rel.endswith("validate-speaking.cjs"):
        text = text.replace("Validated 31 units", "Validated 32 units")
