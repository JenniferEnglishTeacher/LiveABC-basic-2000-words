...
unit_dir = root / "unit-17"
data = json.loads((unit_dir / "unit-17-content.json").read_text(encoding="utf-8"))
source_root = root.parents[1]
shutil.copy2(source_root / "Track 33.mp3", unit_dir / "Track 33.mp3")
shutil.copy2(source_root / "Track 34.mp3", unit_dir / "Track 34.mp3")

template = (root / "unit-04/unit-04-quiz.html").read_text(encoding="utf-8")
html = re.sub(r"<title>[\s\S]*?</title>", "<title>Daydreamer Vocabulary &amp; Reading Quiz</title>", template, count=1)
html = re.sub(r"<h1>[\s\S]*?</h1>", "<h1>Daydreamer</h1>", html, count=1)
html = html.replace("Reading — Track 07", "Reading — Track 33").replace('src="Track 07.mp3"', 'src="Track 33.mp3"')
html = html.replace("Vocabulary — Track 08", "Vocabulary — Track 34").replace('src="Track 08.mp3"', 'src="Track 34.mp3"')
...
(unit_dir / "unit-17-quiz.html").write_text(html, encoding="utf-8")
...
title = doc.add_heading("LiveABC Basic 2000 Words — Unit 17", 0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("Listen to Track 33 and fill in the 19 missing words.")
...
doc.add_page_break()
doc.add_heading("Teacher Answer Key — Unit 17", 0)
...
for i, item in enumerate(data["vocabulary"], 1):
    ...
    run.font.size = Pt(8)
doc.save(unit_dir / "unit-17-test.docx")

sent_path = root / "scripts/speech-sentences.json"
sent = json.loads(sent_path.read_text(encoding="utf-8"))
sent["17"] = [item["sample_sentence"] for item in data["vocabulary"]]
sent_path.write_text(json.dumps(sent, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

index_path = root / "index.html"
index = index_path.read_text(encoding="utf-8")
if "UNIT 17" not in index:
    card = '    <section class="card"><div class="tag">UNIT 17</div><h2>Daydreamer</h2><p>Listening, reading, vocabulary, and target-word spelling and optional read-aloud practice.</p><a class="button" href="unit-17/unit-17-quiz.html">Start Unit 17</a><a class="download" href="unit-17/unit-17-quiz.html#speech">Spell & read aloud · 拼字與朗讀</a><a class="download" href="unit-17/unit-17-test.docx">Download Word test</a></section>\n'
    index = index.replace('  </div>\n  <p class="note">', card + '  </div>\n  <p class="note">')
    index_path.write_text(index, encoding="utf-8")

readme_path = root / "README.md"
readme = readme_path.read_text(encoding="utf-8")
if "Unit 17 —" not in readme:
    readme = readme.replace("- [Unit 16 — Our Sun and Its Twin](unit-16/unit-16-quiz.html)", "- [Unit 16 — Our Sun and Its Twin](unit-16/unit-16-quiz.html)\n- [Unit 17 — Daydreamer](unit-17/unit-17-quiz.html)")
readme = readme.replace("Units 1–16", "Units 1–17")
readme_path.write_text(readme, encoding="utf-8")

for rel in ("scripts/validate-speaking.cjs", "scripts/browser-test.cjs"):
    path = root / rel
    text = path.read_text(encoding="utf-8").replace("n<=16", "n<=17")
    if rel.endswith("browser-test.cjs"):
        text = text.replace("count(),16", "count(),17")
    if rel.endswith("validate-speaking.cjs"):
        text = text.replace("Validated 16 units", "Validated 17 units")
