...
unit_dir = root / "unit-12"
data = json.loads((unit_dir / "unit-12-content.json").read_text(encoding="utf-8"))
source_root = root.parents[1]
shutil.copy2(source_root / "Track 23.mp3", unit_dir / "Track 23.mp3")
shutil.copy2(source_root / "Track 24.mp3", unit_dir / "Track 24.mp3")

template = (root / "unit-04/unit-04-quiz.html").read_text(encoding="utf-8")
html = re.sub(r"<title>[\s\S]*?</title>", "<title>School Trip to Windfield Vocabulary &amp; Reading Quiz</title>", template, count=1)
html = re.sub(r"<h1>[\s\S]*?</h1>", "<h1>School Trip to Windfield</h1>", html, count=1)
html = html.replace("Reading — Track 07", "Reading — Track 23").replace('src="Track 07.mp3"', 'src="Track 23.mp3"')
html = html.replace("Vocabulary — Track 08", "Vocabulary — Track 24").replace('src="Track 08.mp3"', 'src="Track 24.mp3"')
...
(unit_dir / "unit-12-quiz.html").write_text(html, encoding="utf-8")
...
title = doc.add_heading("LiveABC Basic 2000 Words — Unit 12", 0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("Listen to Track 23 and fill in the 14 missing words.")
...
doc.add_page_break()
doc.add_heading("Teacher Answer Key — Unit 12", 0)
...
for i, item in enumerate(data["vocabulary"], 1):
    ...
    run.font.size = Pt(8)
doc.save(unit_dir / "unit-12-test.docx")

sent_path = root / "scripts/speech-sentences.json"
sent = json.loads(sent_path.read_text(encoding="utf-8"))
sent["12"] = [item["sample_sentence"] for item in data["vocabulary"]]
sent_path.write_text(json.dumps(sent, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

index_path = root / "index.html"
index = index_path.read_text(encoding="utf-8")
if "UNIT 12" not in index:
    card = '    <section class="card"><div class="tag">UNIT 12</div><h2>School Trip to Windfield</h2><p>Listening, reading, vocabulary, and target-word spelling and optional read-aloud practice.</p><a class="button" href="unit-12/unit-12-quiz.html">Start Unit 12</a><a class="download" href="unit-12/unit-12-quiz.html#speech">Spell & read aloud · 拼字與朗讀</a><a class="download" href="unit-12/unit-12-test.docx">Download Word test</a></section>\n'
    index = index.replace('  </div>\n  <p class="note">', card + '  </div>\n  <p class="note">')
    index_path.write_text(index, encoding="utf-8")

readme_path = root / "README.md"
readme = readme_path.read_text(encoding="utf-8")
if "Unit 12 —" not in readme:
    readme = readme.replace("- [Unit 11 — Big Town Comic Festival](unit-11/unit-11-quiz.html)", "- [Unit 11 — Big Town Comic Festival](unit-11/unit-11-quiz.html)\n- [Unit 12 — School Trip to Windfield](unit-12/unit-12-quiz.html)")
readme = readme.replace("Units 1–11", "Units 1–12")
readme_path.write_text(readme, encoding="utf-8")

for rel in ("scripts/validate-speaking.cjs", "scripts/browser-test.cjs"):
    path = root / rel
    text = path.read_text(encoding="utf-8").replace("n<=11", "n<=12")
    if rel.endswith("browser-test.cjs"):
        text = text.replace("count(),11", "count(),12")
    if rel.endswith("validate-speaking.cjs"):
        text = text.replace("Validated 11 units", "Validated 12 units")
