...
unit_dir = root / "unit-10"
data = json.loads((unit_dir / "unit-10-content.json").read_text(encoding="utf-8"))
source_root = root.parents[1]
shutil.copy2(source_root / "Track 19.mp3", unit_dir / "Track 19.mp3")
shutil.copy2(source_root / "Track 20.mp3", unit_dir / "Track 20.mp3")

template = (root / "unit-04/unit-04-quiz.html").read_text(encoding="utf-8")
html = re.sub(r"<title>[\s\S]*?</title>", "<title>A Special Visit to London Vocabulary &amp; Reading Quiz</title>", template, count=1)
html = re.sub(r"<h1>[\s\S]*?</h1>", "<h1>A Special Visit to London</h1>", html, count=1)
html = html.replace("Reading — Track 07", "Reading — Track 19").replace('src="Track 07.mp3"', 'src="Track 19.mp3"')
html = html.replace("Vocabulary — Track 08", "Vocabulary — Track 20").replace('src="Track 08.mp3"', 'src="Track 20.mp3"')
...
(unit_dir / "unit-10-quiz.html").write_text(html, encoding="utf-8")
...
title = doc.add_heading("LiveABC Basic 2000 Words — Unit 10", 0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("Listen to Track 19 and fill in the 17 missing words.")
...
doc.add_page_break()
doc.add_heading("Teacher Answer Key — Unit 10", 0)
...
for i, item in enumerate(data["vocabulary"], 1):
    ...
    run.font.size = Pt(8)
doc.save(unit_dir / "unit-10-test.docx")

sent_path = root / "scripts/speech-sentences.json"
sent = json.loads(sent_path.read_text(encoding="utf-8"))
sent["10"] = [item["sample_sentence"] for item in data["vocabulary"]]
sent_path.write_text(json.dumps(sent, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

index_path = root / "index.html"
index = index_path.read_text(encoding="utf-8")
if "UNIT 10" not in index:
    card = '    <section class="card"><div class="tag">UNIT 10</div><h2>A Special Visit to London</h2><p>Listening, reading, vocabulary, and target-word spelling and optional read-aloud practice.</p><a class="button" href="unit-10/unit-10-quiz.html">Start Unit 10</a><a class="download" href="unit-10/unit-10-quiz.html#speech">Spell & read aloud · 拼字與朗讀</a><a class="download" href="unit-10/unit-10-test.docx">Download Word test</a></section>\n'
    index = index.replace('  </div>\n  <p class="note">', card + '  </div>\n  <p class="note">')
    index_path.write_text(index, encoding="utf-8")

readme_path = root / "README.md"
readme = readme_path.read_text(encoding="utf-8")
if "Unit 10 —" not in readme:
    readme = readme.replace("- [Unit 9 — Keith Haring: A Symbol of Love](unit-09/unit-09-quiz.html)", "- [Unit 9 — Keith Haring: A Symbol of Love](unit-09/unit-09-quiz.html)\n- [Unit 10 — A Special Visit to London](unit-10/unit-10-quiz.html)")
readme = readme.replace("Units 1–9", "Units 1–10")
readme_path.write_text(readme, encoding="utf-8")

for rel in ("scripts/validate-speaking.cjs", "scripts/browser-test.cjs"):
    path = root / rel
    text = path.read_text(encoding="utf-8").replace("n<=9", "n<=10")
    if rel.endswith("browser-test.cjs"):
        text = text.replace("count(),9", "count(),10")
    if rel.endswith("validate-speaking.cjs"):
        text = text.replace("Validated 9 units", "Validated 10 units")
