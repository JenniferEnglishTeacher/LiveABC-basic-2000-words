...
unit_dir = root / "unit-27"
data = json.loads((unit_dir / "unit-27-content.json").read_text(encoding="utf-8"))
source_root = root.parents[1]
shutil.copy2(source_root / "Track 53.mp3", unit_dir / "Track 53.mp3")
shutil.copy2(source_root / "Track 54.mp3", unit_dir / "Track 54.mp3")

template = (root / "unit-04/unit-04-quiz.html").read_text(encoding="utf-8")
html = re.sub(r"<title>[\s\S]*?</title>", "<title>The True Value of Wealth Vocabulary &amp; Reading Quiz</title>", template, count=1)
html = re.sub(r"<h1>[\s\S]*?</h1>", "<h1>The True Value of Wealth</h1>", html, count=1)
html = html.replace("Reading — Track 07", "Reading — Track 53").replace('src="Track 07.mp3"', 'src="Track 53.mp3"')
html = html.replace("Vocabulary — Track 08", "Vocabulary — Track 54").replace('src="Track 08.mp3"', 'src="Track 54.mp3"')
...
(unit_dir / "unit-27-quiz.html").write_text(html, encoding="utf-8")
...
title = doc.add_heading("LiveABC Basic 2000 Words — Unit 27", 0); title.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("Listen to Track 53 and fill in the 23 missing words.")
...
doc.add_page_break()
doc.add_heading("Teacher Answer Key — Unit 27", 0)
...
for i, item in enumerate(data["vocabulary"], 1):
    ...
    run.font.size = Pt(8)
doc.save(unit_dir / "unit-27-test.docx")

sent_path = root / "scripts/speech-sentences.json"
sent = json.loads(sent_path.read_text(encoding="utf-8"))
sent["27"] = [item["sample_sentence"] for item in data["vocabulary"]]
sent_path.write_text(json.dumps(sent, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

index_path = root / "index.html"
index = index_path.read_text(encoding="utf-8")
if "UNIT 27" not in index:
    card = '    <section class="card"><div class="tag">UNIT 27</div><h2>The True Value of Wealth</h2><p>Listening, reading, vocabulary, and target-word spelling and optional read-aloud practice.</p><a class="button" href="unit-27/unit-27-quiz.html">Start Unit 27</a><a class="download" href="unit-27/unit-27-quiz.html#speech">Spell & read aloud · 拼字與朗讀</a><a class="download" href="unit-27/unit-27-test.docx">Download Word test</a></section>\n'
    index = index.replace('  </div>\n  <p class="note">', card + '  </div>\n  <p class="note">')
    index_path.write_text(index, encoding="utf-8")

readme_path = root / "README.md"
readme = readme_path.read_text(encoding="utf-8")
if "Unit 27 —" not in readme:
    readme = readme.replace("- [Unit 26 — Protect Yourself from Blue Light](unit-26/unit-26-quiz.html)", "- [Unit 26 — Protect Yourself from Blue Light](unit-26/unit-26-quiz.html)\n- [Unit 27 — The True Value of Wealth](unit-27/unit-27-quiz.html)")
readme = readme.replace("Units 1–26", "Units 1–27")
readme_path.write_text(readme, encoding="utf-8")

for rel in ("scripts/validate-speaking.cjs", "scripts/browser-test.cjs"):
    path = root / rel
    text = path.read_text(encoding="utf-8").replace("n<=26", "n<=27")
    if rel.endswith("browser-test.cjs"):
        text = text.replace("count(),26", "count(),27")
    if rel.endswith("validate-speaking.cjs"):
        text = text.replace("Validated 26 units", "Validated 27 units")
